...
//Finds note tags and seperates top and root chords using "/"
let prep = (line) => {
  return line.replace(/{{[^}]+}}/g, (match) => {
    let chords = match.slice(2,match.length-2);
...
//Changes input into the proper key
let translate = (num) => {
  return all_notes[song_key][num-1]
};

//Seperates text from notes with "!" and checks for minors
let chord = (num, root) => {
  let rootchord = "";
  if (num.match("!")) {
    topchord = translate(num.split("!")[0])
    if (num == 3 || num == 6 || num == 2) {
        topchord += "m";
    };
    topchord += num.split("!")[1];
  } else {
    topchord = translate(num);
    if (num == 3 || num == 6 || num == 2) {
        topchord += "m"
    }
  }
